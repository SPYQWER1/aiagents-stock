...
def triple_screen_filter(context):
    ...
    q = (
        ...
        .filter(valuation.market_cap.between(g.min_market_cap, g.max_market_cap))
...
def check_triple_screen(stock):
    ...
    try:
        ...
        rsi_signal = (
            (rsi_current < g.rsi_oversold)
            or (rsi_prev < g.rsi_low and rsi_current > rsi_prev)
            or (rsi_current < 60 and rsi_current > rsi_prev)
        )
    ...
    for stock in list(context.portfolio.positions.keys()):
        ...
        if highest >= g.take_profit:
            ...
            if drawdown >= allowed_drawdown:
                log.info("【移动止盈】%s 最高%.1f%% 回撤%.1f%%" % (stock, highest * 100, drawdown * 100))
                order_target(stock, 0)
                clean_stock_data(stock)
                continue
