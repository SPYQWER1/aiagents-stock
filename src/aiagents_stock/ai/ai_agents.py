...
class StockAnalysisAgents:
    ...
    def fundamental_analyst_agent(self, stock_info: Dict, financial_data: Dict = None, quarterly_data: Dict = None) -> Dict[str, Any]:
        ...
        if quarterly_data and quarterly_data.get("data_success"):
            ...
            balance_count = (
                quarterly_data.get("balance_sheet", {}).get("periods", 0) if quarterly_data.get("balance_sheet") else 0
            )
            cash_flow_count = (
                quarterly_data.get("cash_flow", {}).get("periods", 0) if quarterly_data.get("cash_flow") else 0
            )
            print(
                f"   ✓ 已获取季报数据：利润表{income_count}期，资产负债表{balance_count}期，现金流量表{cash_flow_count}期"
            )
        else:
