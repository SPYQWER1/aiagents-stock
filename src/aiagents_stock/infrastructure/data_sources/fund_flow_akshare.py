...
class FundFlowAkshareDataFetcher:
    ...
    def get_fund_flow_data(self, symbol):
        ...
        Returns:
            dict: 包含资金流向数据的字典
        """
        data = {"symbol": symbol, "fund_flow_data": None, "data_success": False, "source": "akshare"}
    ...
    def _get_individual_fund_flow(self, symbol, market):
        ...
        try:
            ...
            if df is None or df.empty:
                logger.info("   [Akshare] 未找到资金流向数据，尝试备用数据源...")

                # akshare失败，尝试tushare
                if data_source_manager.tushare_available:
                    try:
                        logger.info("   [Tushare] 正在获取资金流向数据（备用数据源）...")
                        ...
                        if df is not None and not df.empty:
                            ...
                            logger.warning("   [Tushare] ❌ 未找到资金流向数据")
                            return None
            ...
            logger.error(f"   获取资金流向数据异常: {e}", exc_info=True)
            return None
...
""")
                ...
                # 涨跌幅统计
                change_pct_list = [
                    item.get("涨跌幅", 0) for item in data_list if isinstance(item.get("涨跌幅"), (int, float))
                ]
                if change_pct_list:
...
if __name__ == "__main__":
    ...
    # 测试股票
    test_symbols = [("000001", "平安银行"), ("600519", "贵州茅台"), ("000858", "五粮液")]
