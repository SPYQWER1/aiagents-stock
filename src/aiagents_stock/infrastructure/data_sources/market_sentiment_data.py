...
class MarketSentimentDataFetcher:
    ...
    def _calculate_arbr(self, symbol, stock_data=None):
        ...
        try:
            ...
            # 计算AR指标
            df["AR"] = (
                df["HO"].rolling(window=self.arbr_period).sum() / df["OL"].rolling(window=self.arbr_period).sum()
            ) * 100

            # 计算BR指标
            df["BR"] = (
                df["HCY"].rolling(window=self.arbr_period).sum() / df["CYL"].rolling(window=self.arbr_period).sum()
            ) * 100
            ...
            return {
                ...
                "calculation_date": (
                    latest.get("date", datetime.now()).strftime("%Y-%m-%d")
    ...
    def _get_turnover_rate(self, symbol):
        ...
        try:
            ...
            logger.warning(f"   [Akshare] ❌ 获取换手率失败: {e}")

            # akshare失败，尝试tushare
            if data_source_manager.tushare_available:
                try:
                    logger.info("   [Tushare] 正在获取换手率数据（备用数据源）...")
    ...
    def _get_market_index_sentiment(self):
        ...
        try:
            ...
            logger.info("   [Akshare] 正在获取大盘指数数据...")
            # 使用正确的symbol参数
            ...
            if df is not None and not df.empty:
                ...
                if not sh_index.empty:
                    ...
                    try:
                        ...
                        logger.warning(f"   [Akshare] 获取涨跌家数失败: {e}")

                    logger.info("   [Akshare] ✅ 成功获取指数涨跌幅")
                    return {"index_name": "上证指数", "change_percent": change_pct}
        except Exception as e:
            logger.warning(f"   [Akshare] ❌ 获取大盘指数失败: {e}")

            # akshare失败，尝试tushare
            if data_source_manager.tushare_available:
                try:
                    logger.info("   [Tushare] 正在获取大盘指数数据（备用数据源）...")
    ...
    def _get_margin_trading_data(self, symbol):
        ...
        try:
            ...
            logger.error(f"获取融资融券数据失败: {e}", exc_info=True)
        return None
    ...
    def _get_fear_greed_index(self):
        ...
        try:
            ...
            logger.error(f"计算恐慌贪婪指数失败: {e}", exc_info=True)
        return None
