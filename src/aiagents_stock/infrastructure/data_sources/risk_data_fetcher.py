...
class RiskDataFetcher:
    ...
    def get_risk_data(self, symbol: str) -> Dict[str, Any]:
        ...
        Returns:
            包含风险数据的字典
        """
        logger.info(f"正在获取 {symbol} 的风险数据...")
        ...
        try:
            ...
            if lifting_ban and lifting_ban.get("has_data"):
                logger.info("   获取到限售解禁数据")
            else:
                logger.info("   暂无限售解禁数据")
            ...
            if reduction and reduction.get("has_data"):
                logger.info("   获取到大股东减持数据")
            else:
                logger.info("   暂无大股东减持数据")
            ...
            if events and events.get("has_data"):
                logger.info("   获取到重要事件数据")
            else:
                logger.info("   暂无重要事件数据")
            ...
            if (
                ...
                logger.info("风险数据获取完成")
            else:
                logger.warning("未获取到风险相关数据")

        except Exception as e:
            logger.error(f"风险数据获取失败: {str(e)}", exc_info=True)
    ...
    def _get_lifting_ban_data(self, symbol: str) -> Dict[str, Any]:
        """获取限售解禁数据"""
        result = {"has_data": False, "query": f"{symbol}限售解禁", "data": None, "summary": None}
    ...
    def _get_shareholder_reduction_data(self, symbol: str) -> Dict[str, Any]:
        """获取大股东减持公告数据"""
        result = {"has_data": False, "query": f"{symbol}大股东减持公告", "data": None, "summary": None}
    ...
    def _get_important_events_data(self, symbol: str) -> Dict[str, Any]:
        """获取近期重要事件数据"""
        result = {"has_data": False, "query": f"{symbol}近期重要事件", "data": None, "summary": None}
    ...
    def _convert_to_dataframe(self, result) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"   转换DataFrame时出错: {str(e)}", exc_info=True)
            return None
    ...
    def format_risk_data_for_ai(self, risk_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.error(f"格式化风险数据时出错: {str(e)}", exc_info=True)
