...
from aiagents_stock.features.main_force.main_force_analysis import MainForceAnalyzer
from aiagents_stock.features.main_force.main_force_history_ui import display_batch_history
...
def display_main_force_selector():
    ...
    **筛选标准**:
    ...
    with col1:
        date_option = st.selectbox("选择时间区间", ["最近3个月", "最近6个月", "最近1年", "自定义日期"])
        ...
        if date_option == "最近3个月":
            ...
            custom_date = st.date_input("选择开始日期", value=datetime.now() - timedelta(days=90))
            start_date = f"{custom_date.year}年{custom_date.month}月{custom_date.day}日"
            days_ago = None

    with col2:
        final_n = st.slider("最终精选数量", min_value=3, max_value=10, value=5, step=1, help="最终推荐的股票数量")
    ...
    with st.expander("⚙️ 高级筛选参数"):
        ...
        with col2:
            min_cap = st.number_input("最小市值(亿)", min_value=10.0, max_value=500.0, value=50.0, step=10.0)

        with col3:
            max_cap = st.number_input("最大市值(亿)", min_value=50.0, max_value=50000.0, value=5000.0, step=100.0)
...
def display_analysis_results(result: dict, analyzer):
    ...
    if result["final_recommendations"]:
        ...
        for rec in result["final_recommendations"]:
            with st.expander(f"【第{rec['rank']}名】{rec['symbol']} - {rec['name']}", expanded=(rec["rank"] <= 3)):
    ...
    if analyzer and analyzer.raw_stocks is not None and not analyzer.raw_stocks.empty:
        ...
        with col_batch2:
            batch_count = st.selectbox(
                "分析数量", options=[10, 20, 30, 50], index=1, help="选择分析主力资金净流入前N只股票"  # 默认20只
            )
...
def run_main_force_batch_analysis():
    ...
    st.info(
        f"即将分析 {len(stock_codes)} 只股票：{', '.join(stock_codes[:10])}{'...' if len(stock_codes) > 10 else ''}"
    )
    ...
    if start_analysis:
        ...
        with st.expander("🔍 调试信息", expanded=True):
            ...
            st.write(
                f"**代码格式检查**: {'✅ 无后缀，格式正确' if all('.' not in str(c) for c in stock_codes) else '❌ 包含后缀，可能有问题'}"
            )
        ...
        if success_count > 0:
            st.success(
                f"✅ 批量分析完成！成功 {success_count} 只，失败 {failed_count} 只，耗时 {elapsed_time/60:.1f} 分钟"
            )
        else:
...
def display_main_force_batch_results(batch_results):
    ...
    if successful_results:
        ...
        for result in successful_results:
            ...
            rating_emoji = {"强烈买入": "🔥", "买入": "✅", "持有": "⏸️", "卖出": "⚠️", "强烈卖出": "🚫"}.get(
                rating, "❓"
            )
    ...
    if failed_results:
        ...
        for result in failed_results:
            failed_data.append({"股票代码": result.get("symbol", ""), "失败原因": result.get("error", "未知错误")})
