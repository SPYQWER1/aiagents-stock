    ...
    def run_full_analysis(
        ...
        try:
            ...
            success, raw_data, message = self.selector.get_main_force_stocks(
                start_date=start_date, days_ago=days_ago, min_market_cap=min_market_cap, max_market_cap=max_market_cap
            )
...
"""
    ...
    def _prepare_data_table(self, df: pd.DataFrame, focus: str = "all") -> str:
        ...
        if focus == "fundamental" or focus == "all":
            fundamental_cols = [
                col
                for col in df.columns
                if any(keyword in col for keyword in ["市盈率", "市净率", "营收", "净利润", "评分"])
            ]
    ...
    def _select_best_stocks(
        self, df: pd.DataFrame, fund_analysis: str, industry_analysis: str, fundamental_analysis: str, final_n: int = 5
    ) -> List[Dict]:
        """综合三位分析师的意见，精选最优标的"""
