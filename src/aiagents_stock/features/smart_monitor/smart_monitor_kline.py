...
class SmartMonitorKline:
    ...
    def create_kline_with_decisions(
        ...
        try:
            ...
            if show_volume:
                ...
                fig = make_subplots(rows=1, cols=1, subplot_titles=(f"{stock_code} {stock_name}",))
            ...
            # 更新x轴
            fig.update_xaxes(title_text="日期", row=2 if show_volume else 1, col=1)
        ...
        try:
            ...
            for period, color in zip(ma_periods, ma_colors):
                if len(kline_data) >= period:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=kline_data["日期"], y=ma, name=f"MA{period}", line=dict(color=color, width=1), opacity=0.7
                        ),
                        row=row,
                        col=col,
                    )
    ...
    def _add_ai_decision_markers(self, fig, kline_data: pd.DataFrame, ai_decisions: List[Dict], row: int, col: int):
        """在K线图上添加AI决策标注"""
        try:
            # 决策类型映射
            action_config = {
                "buy": {"symbol": "triangle-up", "color": "#ef5350", "text": "买入", "size": 15},
                "sell": {"symbol": "triangle-down", "color": "#26a69a", "text": "卖出", "size": 15},
                "add_position": {"symbol": "triangle-up", "color": "#ff9800", "text": "加仓", "size": 12},
                "reduce_position": {"symbol": "triangle-down", "color": "#9c27b0", "text": "减仓", "size": 12},
                "hold": {"symbol": "circle", "color": "#607d8b", "text": "持有", "size": 8},
            }
    ...
    def _add_volume(self, fig, kline_data: pd.DataFrame, row: int, col: int):
        ...
        try:
            ...
            fig.add_trace(
                go.Bar(
                    x=kline_data["日期"], y=kline_data["成交量"], name="成交量", marker_color=colors, showlegend=False
                ),
                row=row,
                col=col,
            )
    ...
    def _create_empty_figure(self, stock_code: str, stock_name: str, height: int) -> go.Figure:
        ...
        fig.update_layout(height=height, xaxis=dict(visible=False), yaxis=dict(visible=False), template="plotly_white")
        return fig
    ...
    def _get_kline_from_tushare(self, stock_code: str, days: int, ts_pro) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # 获取日K线数据（前复权）
            df = ts_pro.daily(ts_code=ts_code, start_date=start_date, end_date=end_date, adj="qfq")
