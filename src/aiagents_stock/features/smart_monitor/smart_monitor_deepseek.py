...
class SmartMonitorDeepSeek:
    ...
    def chat_completion(
        self, messages: List[Dict], model: str = "deepseek-chat", temperature: float = 0.7, max_tokens: int = 2000
    ) -> Dict:
        """
        调用DeepSeek API
    ...
    def analyze_stock_and_decide(
        ...
        prompt = self._build_a_stock_prompt(
            stock_code, market_data, account_info, has_position, session_info, position_cost, position_quantity
        )
...
"""
        ...
        try:
            ...
            return {"success": True, "decision": decision, "raw_response": ai_response}

        except Exception as e:
            self.logger.error(f"AI决策失败: {e}")
            return {"success": False, "error": str(e)}

    def _build_a_stock_prompt(
        self,
        stock_code: str,
        market_data: Dict,
        account_info: Dict,
