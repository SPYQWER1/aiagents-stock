...
from aiagents_stock.core.config_manager import config_manager  # 复用主程序的配置管理器
from aiagents_stock.features.smart_monitor.smart_monitor_data import SmartMonitorDataFetcher
from aiagents_stock.features.smart_monitor.smart_monitor_db import SmartMonitorDB
from aiagents_stock.features.smart_monitor.smart_monitor_deepseek import SmartMonitorDeepSeek
from aiagents_stock.features.smart_monitor.smart_monitor_qmt import SmartMonitorQMT, SmartMonitorQMTSimulator
from aiagents_stock.notifications.notification_service import notification_service  # 复用主程序的通知服务


class SmartMonitorEngine:
    """智能盯盘引擎"""

    def __init__(self, deepseek_api_key: str = None, qmt_account_id: str = None, use_simulator: bool = None):
        """
        初始化智能盯盘引擎
        ...
        if use_simulator:
            ...
            if qmt_account_id:
                ...
                if success:
                    ...
                    self.logger.warning("连接miniQMT失败，切换到模拟模式")
                    self.qmt = SmartMonitorQMTSimulator()
    ...
    def analyze_stock(
        ...
        try:
            ...
            if has_position and position_cost > 0 and position_quantity > 0:
                # 使用用户设置的持仓信息
                self.logger.info(
                    f"[{stock_code}] 使用监控任务设置的持仓: {position_quantity}股 @ {position_cost:.2f}元"
                )
            else:
                # 从QMT获取持仓
                ...
                if has_position:
                    ...
                    self.logger.info(
                        f"[{stock_code}] 从QMT获取持仓: {position_quantity}股, "
            ...
            if not ai_result["success"]:
                return {"success": False, "error": "AI决策失败", "details": ai_result}

            decision = ai_result["decision"]

            self.logger.info(f"[{stock_code}] AI决策: {decision['action']} " f"(信心度: {decision['confidence']}%)")
            ...
            if auto_trade and session_info["can_trade"]:
                execution_result = self._execute_decision(
                    stock_code=stock_code, decision=decision, market_data=market_data, has_position=has_position
                )
    ...
    def _execute_decision(self, stock_code: str, decision: Dict, market_data: Dict, has_position: bool) -> Dict:
        ...
        try:
            if action == "BUY" and not has_position:
                ...
                # 持有，不操作
                return {"success": True, "action": "HOLD", "message": "AI建议持有，未执行交易"}
    ...
    def _execute_buy(self, stock_code: str, decision: Dict, market_data: Dict) -> Dict:
        ...
        try:
            ...
            if quantity < 100:
                return {"success": False, "error": f"资金不足，最少需要买入100股（约{current_price * 100:.2f}元）"}

            # 执行买入
            result = self.qmt.buy_stock(
                stock_code=stock_code, quantity=quantity, price=current_price, order_type="market"
            )
    ...
    def _execute_sell(self, stock_code: str, decision: Dict, market_data: Dict) -> Dict:
        ...
        try:
            ...
            if can_sell <= 0:
                return {"success": False, "error": "T+1限制，今天买入的股票明天才能卖出"}

            # 执行卖出
            current_price = market_data["current_price"]
            result = self.qmt.sell_stock(
                stock_code=stock_code, quantity=can_sell, price=current_price, order_type="market"
            )
            ...
            if result["success"]:
                ...
                self.logger.info(
                    f"[{stock_code}] 卖出成功: {can_sell}股 @ {current_price:.2f}元, " f"盈亏: {profit_loss:+.2f}元"
                )
    ...
    def _send_notification(
        ...
        try:
            ...
            # 简化的AI决策内容（提取核心信息）
            reasoning_summary = (
                decision["reasoning"][:150] + "..." if len(decision["reasoning"]) > 150 else decision["reasoning"]
            )
...
"""
            ...
            notification_data = {
                ...
                "change_amount": (
                    f"{market_data.get('change_amount', 0):+.2f}" if market_data.get("change_amount") else "N/A"
                ),
                "volume": market_data.get("volume", "N/A"),
                "turnover_rate": (
                    f"{market_data.get('turnover_rate', 0):.2f}" if market_data.get("turnover_rate") else "N/A"
                ),
                # 持仓信息
                "position_status": "已持仓" if has_position else "未持仓",
                "position_cost": f"{position_cost:.2f}" if has_position and position_cost else "N/A",
                "profit_loss_pct": (
                    f"{((market_data.get('current_price', 0) - position_cost) / position_cost * 100):+.2f}"
