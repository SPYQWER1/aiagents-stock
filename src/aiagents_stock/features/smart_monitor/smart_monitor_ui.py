...
from aiagents_stock.core.config_manager import config_manager  # 使用主程序的配置管理器
from aiagents_stock.features.smart_monitor.smart_monitor_db import SmartMonitorDB
from aiagents_stock.features.smart_monitor.smart_monitor_engine import SmartMonitorEngine
...
def smart_monitor_ui():
    ...
    # 创建标签页
    tabs = st.tabs(["📊 实时分析", "🎯 监控任务", "📈 持仓管理", "📜 历史记录", "⚙️ 系统设置"])
...
def render_realtime_analysis():
    ...
    with col1:
        stock_code = st.text_input("输入股票代码", placeholder="例如: 600519", help="输入6位股票代码")

    with col2:
        auto_trade = st.checkbox("自动交易", value=False, help="开启后AI会自动执行交易决策")
    ...
    if st.button("🔍 开始分析", type="primary"):
        ...
        with st.spinner("正在分析..."):
            ...
            result = engine.analyze_stock(stock_code=stock_code, auto_trade=auto_trade, notify=True)
...
def display_analysis_result(result: dict):
    ...
    action_display = f"{action_emoji.get(action, '')} {action}".strip()

    col1.metric("决策", action_display, delta=None)
    ...
    with st.expander("➕ 添加新监控任务", expanded=True):
        ...
        with st.form("add_monitor_task_form", clear_on_submit=False):
            ...
            with col1:
                ...
                position_quantity = st.number_input(
                    "持仓数量(股)",
                    min_value=100,
                    value=100,
                    step=100,
            ...
            with col2:
                auto_trade = st.checkbox("自动交易", value=False, help="AI决策后自动执行交易")
                trading_hours_only = st.checkbox(
                    "仅交易时段监控",
                    value=True,
                    help="开启后，只在交易日的交易时段（9:30-11:30, 13:00-15:00）进行AI分析",
                )
                position_size = st.slider("仓位百分比(%)", 5, 50, 20, help="新建仓位时使用的资金比例")
...
def render_position_management():
    ...
    selected_stock = st.selectbox("选择股票", options=[f"{p['stock_code']} {p['stock_name']}" for p in positions])
...
def _render_task_kline_and_decisions(task: Dict, db: SmartMonitorDB, engine):
    ...
    """
    from aiagents_stock.features.smart_monitor.smart_monitor_data import SmartMonitorDataFetcher
    from aiagents_stock.features.smart_monitor.smart_monitor_kline import SmartMonitorKline
    ...
    with col_chart:
        ...
        try:
            ...
            if kline_data is not None and not kline_data.empty:
                ...
                ai_decisions = db.get_ai_decisions(stock_code=stock_code, limit=100)
                ...
                if ai_decisions:
                    ...
                    ai_decisions = [d for d in ai_decisions if d.get("decision_time", "").split()[0] >= start_date]
    ...
    with col_decisions:
        ...
        # 获取最近的AI决策（最近5条）
        try:
            recent_decisions = db.get_ai_decisions(stock_code=stock_code, limit=5)
