...
class SmartMonitorQMT:
    ...
    def get_account_info(self) -> Dict:
        ...
        if not self.connected or not self.account:
            return {"available_cash": 0, "total_value": 0, "positions_count": 0, "total_profit_loss": 0}
            ...
            for pos in positions:
                if pos.stock_code == stock_code:
                    ...
                    return {
                        ...
                        "profit_loss_pct": (
                            (pos.last_price - pos.avg_price) / pos.avg_price * 100 if pos.avg_price > 0 else 0
                        ),
    ...
    def buy_stock(self, stock_code: str, quantity: int, price: float = 0, order_type: str = "market") -> Dict:
        """
        买入股票
        ...
        if not self.connected:
            return {"success": False, "error": "miniQMT未连接", "message": "模拟模式：买入订单已记录但未实际执行"}

        # 检查数量是否是100的整数倍
        if quantity % 100 != 0:
            return {"success": False, "error": "A股买入数量必须是100的整数倍（1手=100股）"}
        ...
        try:
            ...
            if order_id > 0:
                ...
                return {"success": False, "error": f"下单失败，订单号: {order_id}"}

        except Exception as e:
            self.logger.error(f"买入股票失败 {stock_code}: {e}")
            return {"success": False, "error": str(e)}

    def sell_stock(self, stock_code: str, quantity: int, price: float = 0, order_type: str = "market") -> Dict:
        """
        卖出股票
        ...
        """
        if not self.connected:
            return {"success": False, "error": "miniQMT未连接", "message": "模拟模式：卖出订单已记录但未实际执行"}
        ...
        try:
            ...
            if quantity > position["can_sell"]:
                return {"success": False, "error": f'可卖数量不足（可卖: {position["can_sell"]}股，T+1限制）'}
...
class SmartMonitorQMTSimulator:
    ...
    def sell_stock(self, stock_code: str, quantity: int, price: float = 0, order_type: str = "market") -> Dict:
        ...
        return {"success": True, "order_id": len(self.orders), "profit_loss": profit_loss, "message": "模拟卖出成功"}
