...
class SmartMonitorDataFetcher:
    ...
    def __init__(self, use_tdx: bool = None, tdx_base_url: str = None):
        ...
        # TDX数据源配置
        if use_tdx is None:
            from aiagents_stock.core.config import TDX_CONFIG
    ...
    def get_realtime_quote(self, stock_code: str, retry: int = 1) -> Optional[Dict]:
        ...
        for attempt in range(retry):
            try:
                ...
                if attempt < retry - 1:
                    self.logger.warning(
                        f"AKShare获取失败 {stock_code}，第{attempt+1}次重试... 错误: {type(e).__name__}: {str(e)[:50]}"
                    )
        ...
        for attempt in range(retry):
            try:
                ...
                df = ak.stock_zh_a_hist(
                    symbol=stock_code, period=period, start_date=start_date, end_date=end_date, adjust="qfq"  # 前复权
                )
                ...
                if attempt < retry - 1:
                    self.logger.warning(
                        f"AKShare获取历史数据失败 {stock_code}，第{attempt+1}次重试... 错误: {type(e).__name__}: {str(e)[:50]}"
                    )
                    time.sleep(1)
                else:
    ...
    def get_main_force_flow(self, stock_code: str, retry: int = 2) -> Optional[Dict]:
        ...
        for attempt in range(retry):
            try:
                ...
                if attempt < retry - 1:
                    self.logger.warning(
                        f"AKShare获取资金流向失败 {stock_code}，第{attempt+1}次重试... 错误: {type(e).__name__}"
                    )
    ...
    def _get_main_force_from_tushare(self, stock_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            error_msg = str(e)
            if "权限" in error_msg or "积分" in error_msg:
                self.logger.warning("⚠️ Tushare资金流向接口需要120积分，当前积分不足")
                self.logger.info("💡 获取积分方法：")
                self.logger.info("   1. 完善个人信息 +100积分")
