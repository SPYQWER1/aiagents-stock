...
def get_default_db_path() -> str:
    """获取默认数据库路径（基于项目根目录）"""
    # src/aiagents_stock/features/longhubang/longhubang_db.py -> ... -> project_root
    current_dir = Path(__file__).resolve().parent
    project_root = current_dir.parent.parent.parent.parent
    return str(project_root / "database_files" / "longhubang.db")
...
class LonghubangDatabase:
    ...
    def save_analysis_report(self, data_date_range, analysis_content, recommended_stocks, summary, full_result=None):
        """
        保存AI分析报告（完整版）
    ...
    def update_stock_tracking(self, analysis_id, stock_code, current_price, status, notes=None):
        ...
        """,
            (current_price, status, notes, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), analysis_id, stock_code),
        )
