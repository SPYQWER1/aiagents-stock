...
from aiagents_stock.features.longhubang.longhubang_engine import LonghubangEngine
from aiagents_stock.features.longhubang.longhubang_pdf import LonghubangPDFGenerator
...
def display_longhubang():
    ...
    # 创建标签页
    tab1, tab2, tab3 = st.tabs(["📊 龙虎榜分析", "📚 历史报告", "📈 数据统计"])
...
def display_analysis_tab():
    ...
    with col1:
        analysis_mode = st.selectbox("分析模式", ["指定日期", "最近N天"], help="选择分析特定日期还是最近几天的数据")

    with col2:
        if analysis_mode == "指定日期":
            selected_date = st.date_input(
                "选择日期", value=datetime.now() - timedelta(days=1), help="选择要分析的龙虎榜日期"
            )
        else:
            days = st.number_input("最近天数", min_value=1, max_value=10, value=1, help="分析最近N天的龙虎榜数据")

    with col3:
        # 导入model_config.py中定义的model_options
        from aiagents_stock.core.model_config import model_options as app_model_options
...
def display_analysis_results(result):
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["🏆 AI评分排名", "🎯 推荐股票", "🤖 AI分析师报告", "📊 数据详情", "📈 可视化图表"]
    )
...
def display_scoring_ranking(result):
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "综合评分": st.column_config.NumberColumn("综合评分", format="%.1f", help="总分100分"),
            "资金含金量": st.column_config.ProgressColumn("资金含金量", format="%d分", min_value=0, max_value=30),
            "净买入额": st.column_config.ProgressColumn("净买入额", format="%d分", min_value=0, max_value=25),
            "卖出压力": st.column_config.ProgressColumn("卖出压力", format="%d分", min_value=0, max_value=20),
            "机构共振": st.column_config.ProgressColumn("机构共振", format="%d分", min_value=0, max_value=15),
            "加分项": st.column_config.ProgressColumn("加分项", format="%d分", min_value=0, max_value=10),
    ...
    with col_batch2:
        batch_count = st.selectbox(
            "分析数量", options=[3, 5, 10], index=0, help="选择分析前N只股票", key="batch_count_selector"
        )
    ...
    with col2:
        ...
        if len(top10_df) > 0:
            ...
            fig2.update_layout(
                ...
                height=400,
                legend=dict(orientation="h", yanchor="auto", y=-0.2, xanchor="center", x=0.5),
            )
...
def display_visualizations(result):
    ...
    if summary.get("hot_concepts"):
        ...
        fig = px.pie(df_concepts, values="次数", names="概念", title="热门概念出现次数分布")
        st.plotly_chart(fig, config={"displayModeBar": False}, width="stretch")
...
def display_history_tab():
    ...
    try:
        ...
        for idx, row in reports_df.iterrows():
            ...
            # 创建展开面板
            with st.expander(f"📄 报告 #{report_id} | {analysis_date} | 数据范围: {data_date_range}", expanded=False):
                # 获取完整报告详情
                ...
                if analysis_content_parsed and isinstance(analysis_content_parsed, dict):
                    ...
                    if scoring_ranking:
                        ...
                        if "排名" in df_scoring.columns:
                            df_scoring["排名"] = (
                                pd.to_numeric(df_scoring["排名"], errors="coerce").fillna(0).astype(int)
                            )
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                ...
                                "资金含金量": st.column_config.ProgressColumn(
                                    "资金含金量", format="%d分", min_value=0, max_value=30
                                ),
                                "净买入额": st.column_config.ProgressColumn(
                                    "净买入额", format="%d分", min_value=0, max_value=25
                                ),
                                "卖出压力": st.column_config.ProgressColumn(
                                    "卖出压力", format="%d分", min_value=0, max_value=20
                                ),
                                "机构共振": st.column_config.ProgressColumn(
                                    "机构共振", format="%d分", min_value=0, max_value=15
                                ),
                                "加分项": st.column_config.ProgressColumn(
                                    "加分项", format="%d分", min_value=0, max_value=10
                                ),
                                "顶级游资": st.column_config.NumberColumn("顶级游资", format="%d家"),
                                "买方数": st.column_config.NumberColumn("买方数", format="%d家"),
                                "机构参与": st.column_config.TextColumn("机构参与"),
                ...
                with col_export1:
                    if st.button("📥 导出为PDF", key=f"export_pdf_{report_id}"):
                        st.info("PDF导出功能开发中...")

                with col_export2:
                    # 使用session_state来管理按钮状态，避免需要点击两次的问题
                    load_key = f"load_report_{report_id}"
                    if st.button("📋 加载到分析页", key=load_key):
                        # 将历史报告加载到当前分析结果中
                ...
                with col_export3:
                    ...
                    if st.button("🗑️ 删除报告", key=delete_key, type="secondary"):
                ...
                if st.session_state.get(f"confirm_delete_{report_id}", False):
                    ...
                    with col_confirm1:
                        if st.button("✅ 确认删除", key=f"confirm_delete_yes_{report_id}", type="primary"):
                            try:
                                # 调用数据库删除方法 - 修复属性名
                    ...
                    with col_confirm2:
                        if st.button("❌ 取消", key=f"confirm_delete_no_{report_id}"):
                            # 清除确认状态
                            if f"confirm_delete_{report_id}" in st.session_state:
                                del st.session_state[f"confirm_delete_{report_id}"]
                            st.rerun()
...
def run_longhubang_batch_analysis():
    ...
    with col2:
        if analysis_mode == "parallel":
            max_workers = st.number_input("并行线程数", min_value=2, max_value=5, value=3, help="同时分析的股票数量")
        else:
            max_workers = 1
        ...
        if analysis_mode == "sequential":
            ...
            for i, code in enumerate(stock_codes):
                ...
                try:
                    ...
                    results.append({"code": code, "result": {"success": False, "error": str(e)}})
...
def display_longhubang_batch_results(batch_results: dict):
    ...
    for item in success_results:
        ...
        with st.expander(
            f"{rating_color} {code} {stock_info.get('name', '')} - {rating} (信心度: {confidence})", expanded=False
        ):
            col1, col2, col3 = st.columns(3)
            ...
            # 添加到监测按钮
            if st.button("➕ 加入监测", key=f"add_monitor_{code}"):
...
if __name__ == "__main__":
    st.set_page_config(page_title="智瞰龙虎", page_icon="🎯", layout="wide")
