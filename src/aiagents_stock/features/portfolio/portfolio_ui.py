...
from aiagents_stock.features.portfolio.portfolio_scheduler import portfolio_scheduler
...
def display_portfolio_manager():
    ...
    # 创建标签页
    tab1, tab2, tab3, tab4 = st.tabs(["📝 持仓管理", "🔄 批量分析", "⏰ 定时任务", "📈 分析历史"])
...
def display_portfolio_stocks():
    ...
    with col3:
        total_cost = sum(
            s.get("cost_price", 0) * s.get("quantity", 0) for s in stocks if s.get("cost_price") and s.get("quantity")
        )
            ...
            with st.form(key=f"edit_form_{code}"):
                ...
                with col_a:
                    new_cost = st.number_input(
                        "成本价", value=cost_price if cost_price else 0.0, min_value=0.0, step=0.01
                    )
                    new_quantity = st.number_input("持仓数量", value=quantity if quantity else 0, min_value=0, step=100)
...
def display_add_stock_form():
    ...
    with st.form(key="add_stock_form"):
        ...
        with col1:
            code = st.text_input(
                "股票代码*", placeholder="例如: 600519.SH 或 000001.SZ", help="必填，格式：代码.市场（SH/SZ/HK/US）"
            )
            name = st.text_input("股票名称", placeholder="例如: 贵州茅台", help="可选，留空将自动获取")

        with col2:
            cost_price = st.number_input("成本价", min_value=0.0, step=0.01, help="可选，用于计算收益")
            quantity = st.number_input("持仓数量", min_value=0, step=100, help="可选，单位：股")
...
def display_batch_analysis():
    ...
    with col3:
        if analysis_mode == "parallel":
            max_workers = st.number_input("并行线程数", min_value=2, max_value=10, value=3, help="同时分析的股票数量")
        else:
            max_workers = 1
    ...
    with col_a:
        auto_sync = st.checkbox("自动同步到监测", value=True, help="分析完成后自动将评级结果同步到实时监测列表")

    with col_b:
        send_notification = st.checkbox("发送完成通知", value=True, help="通过邮件或Webhook发送分析完成通知")
        ...
        with st.spinner("正在批量分析持仓股票..."):
            ...
            try:
                ...
                result = portfolio_manager.batch_analyze_portfolio(
                    mode=analysis_mode, max_workers=max_workers, progress_callback=update_progress
                )
                ...
                if auto_sync:
                    with st.spinner("正在同步到监测列表..."):
                        ...
                        if monitors_to_sync:
                            ...
                            st.info(
                                f"📊 监测同步: 新增 {sync_result.get('added', 0)} 只, 更新 {sync_result.get('updated', 0)} 只"
                            )
                        else:
                ...
                if send_notification:
                    ...
                    notification_service.send_portfolio_analysis_notification(
                        result, sync_result if auto_sync else None
                    )
                    st.info("✉️ 已发送完成通知")
...
def display_scheduler_management():
    ...
    with st.expander("➕ 添加定时时间", expanded=False):
        ...
        with col_input:
            new_time = st.time_input(
                "选择时间", value=datetime.strptime("15:05", "%H:%M").time(), help="添加新的每日分析时间"
            )
        with col_add:
            st.write("")  # 占位，对齐按钮
            st.write("")
    ...
    with st.form(key="scheduler_config_form"):
        ...
        auto_sync_monitor = st.checkbox(
            "自动同步到监测", value=portfolio_scheduler.auto_monitor_sync, help="分析完成后自动将结果同步到实时监测列表"
        )
        send_notification = st.checkbox(
            "发送完成通知", value=portfolio_scheduler.notification_enabled, help="通过邮件或Webhook发送分析结果"
        )
        ...
        with col_reset:
            if st.form_submit_button("🔄 恢复默认"):
                ...
                portfolio_scheduler.update_config(
                    analysis_mode="sequential", max_workers=1, auto_sync_monitor=True, send_notification=True
                )
                st.success("已恢复默认配置！")
                time.sleep(0.5)
                st.rerun()
...
def display_analysis_history():
    ...
    selected_code = st.selectbox("选择股票", options=["全部"] + stock_codes, help="查看特定股票的分析历史")
    ...
    if selected_code == "全部":
        ...
        if stock:
            history_list = portfolio_manager.db.get_latest_analysis_history(stock["id"], limit=20)
...
def display_history_record(record: Dict):
    ...
    with st.expander(f"{rating_icon} {code} {name} - {rating} | {analysis_time}", expanded=False):
        col1, col2, col3 = st.columns(3)
