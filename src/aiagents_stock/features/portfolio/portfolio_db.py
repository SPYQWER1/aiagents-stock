...
def get_default_db_path() -> str:
    """获取默认数据库路径（基于项目根目录）"""
    # src/aiagents_stock/features/portfolio/portfolio_db.py -> ... -> project_root
    current_dir = Path(__file__).resolve().parent
    project_root = current_dir.parent.parent.parent.parent
    return str(project_root / "database_files" / "portfolio_stocks.db")
    ...
    def add_stock(
        ...
        try:
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (code, name, cost_price, quantity, note, auto_monitor, datetime.now(), datetime.now()),
            )
        ...
        try:
            ...
            for i in range(1, len(rows)):
                ...
                if prev_rating != curr_rating:
                    changes.append((rows[i]["analysis_time"], prev_rating, curr_rating))
