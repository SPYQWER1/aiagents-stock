...
# 导入必要的模块
from aiagents_stock.features.portfolio.portfolio_db import portfolio_db
...
class PortfolioManager:
    ...
    def add_stock(
        ...
        try:
            ...
            self.logger.info(f"添加持仓股票成功: {code} {name}")
            return True, f"添加持仓股票成功: {code} {name}", stock_id

        except Exception as e:
            self.logger.error(f"添加持仓股票失败: {e}")
    ...
    def update_stock(self, stock_id: int, **kwargs) -> Tuple[bool, str]:
        ...
        try:
            ...
            if success:
                self.logger.info(f"更新持仓股票成功 (ID: {stock_id})")
                return True, "更新成功"
            else:
                self.logger.warning(f"更新持仓股票失败: 未找到股票ID {stock_id}")
                return False, f"未找到股票ID: {stock_id}"
        except Exception as e:
            self.logger.error(f"更新持仓股票失败: {e}")
    ...
    def delete_stock(self, stock_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if success:
                self.logger.info(f"删除持仓股票成功 (ID: {stock_id})")
                return True, "删除成功"
            else:
                self.logger.warning(f"删除持仓股票失败: 未找到股票ID {stock_id}")
                return False, f"未找到股票ID: {stock_id}"
        except Exception as e:
            self.logger.error(f"删除持仓股票失败: {e}")
    ...
    def batch_analyze_sequential(
        self, stock_codes: List[str], period="1y", selected_agents: List[str] = None, progress_callback=None
    ) -> Dict:
        """
        顺序批量分析（逐只分析）
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info(f"开始批量分析 (顺序模式): {len(stock_codes)}只股票")
        self.logger.info("=" * 60)
        ...
        for i, code in enumerate(stock_codes, 1):
            self.logger.info(f"--- 分析进度: {i}/{len(stock_codes)} ---")
            ...
            try:
                ...
                self.logger.error(f"股票 {code} 分析失败: {str(e)}")
        ...
        self.logger.info(f"成功: {len(results)}只, 失败: {len(failed)}只, 耗时: {elapsed_time:.1f}秒")
        self.logger.info("=" * 60)
    ...
    def batch_analyze_parallel(
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info(f"开始批量分析 (并行模式): {len(stock_codes)}只股票, 并发数: {max_workers}")
        self.logger.info("=" * 60)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任务
            future_to_code = {
                executor.submit(self.analyze_single_stock, code, period, selected_agents): code for code in stock_codes
            }
            ...
            for future in as_completed(future_to_code):
                ...
                try:
                    ...
                    if result.get("success"):
                        results.append({"code": code, "result": result})
                        self.logger.info(f"[{completed}/{len(stock_codes)}] {code} 分析完成")
                        if progress_callback:
                            progress_callback(completed, len(stock_codes), code, "success")
                    else:
                        failed.append({"code": code, "error": result.get("error", "未知错误")})
                        self.logger.warning(f"[{completed}/{len(stock_codes)}] {code} 分析失败: {result.get('error')}")
                        if progress_callback:
                            progress_callback(completed, len(stock_codes), code, "failed")

                except Exception as e:
                    failed.append({"code": code, "error": str(e)})
                    self.logger.error(f"[{completed}/{len(stock_codes)}] {code} 分析异常: {str(e)}")
                    if progress_callback:
    ...
    def batch_analyze_portfolio(
        ...
        if not stocks:
            return {"success": False, "error": "没有持仓股票"}
    ...
    def save_analysis_results(self, analysis_results: Dict) -> List[int]:
        ...
        if not analysis_results.get("success"):
            self.logger.warning("分析未成功，跳过保存")
            return saved_ids
        ...
        for item in analysis_results.get("results", []):
            ...
            if not stock:
                self.logger.warning(f"未找到持仓股票: {code}，跳过保存")
                continue
            ...
            try:
                ...
                self.logger.error(f"保存分析结果失败 ({code}): {str(e)}")

        self.logger.info(f"保存分析结果: {len(saved_ids)}条记录")
        return saved_ids
