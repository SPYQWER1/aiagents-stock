...
from aiagents_stock.features.portfolio.portfolio_manager import portfolio_manager
from aiagents_stock.notifications.notification_service import NotificationService
...
class PortfolioScheduler:
    ...
    def set_schedule_time(self, time_str: str):
        ...
        try:
            ...
            self.logger.info(f"设置定时分析时间: {time_str}")
            ...
            self.logger.error(f"无效的时间格式: {time_str}，应为 HH:MM")
    ...
    def add_schedule_time(self, time_str: str) -> bool:
        ...
        try:
            ...
            if time_str in self.schedule_times:
                self.logger.warning(f"定时时间 {time_str} 已存在")
                return False

            self.schedule_times.append(time_str)
            self.schedule_times.sort()  # 保持时间顺序
            self.logger.info(f"添加定时时间: {time_str}")
    ...
    def remove_schedule_time(self, time_str: str) -> bool:
        ...
        if time_str in self.schedule_times:
            ...
            self.logger.info(f"删除定时时间: {time_str}")
            ...
            self.logger.warning(f"定时时间 {time_str} 不存在")
            return False
    ...
    def set_schedule_times(self, times: list):
        ...
        for time_str in times:
            try:
                ...
                self.logger.warning(f"跳过无效时间: {time_str}")

        if valid_times:
            self.schedule_times = sorted(valid_times)
            self.logger.info(f"设置定时时间: {', '.join(self.schedule_times)}")

            # 如果调度器正在运行，重新调度
            if self._is_running:
                self._reschedule()
        else:
            self.logger.error("没有有效的时间配置")
    ...
    def set_analysis_mode(self, mode: str):
        ...
        if mode in ["sequential", "parallel"]:
            ...
            self.logger.info(f"设置分析模式: {mode}")
        else:
            self.logger.error(f"无效的分析模式: {mode}")

    def set_auto_monitor_sync(self, enabled: bool):
        """设置是否启用自动监测同步"""
        self.auto_monitor_sync = enabled
        self.logger.info(f"自动监测同步: {'启用' if enabled else '禁用'}")

    def set_notification_enabled(self, enabled: bool):
        """设置是否启用通知"""
        self.notification_enabled = enabled
        self.logger.info(f"通知推送: {'启用' if enabled else '禁用'}")

    def set_selected_agents(self, agents: Optional[list]):
        """设置参与分析的AI分析师"""
        self.selected_agents = agents
        if agents:
            self.logger.info(f"选择分析师: {', '.join(agents)}")
        else:
            self.logger.info("选择分析师: 全部")

    def _scheduled_job(self):
        """定时任务执行的作业"""
        self.logger.info("=" * 60)
        self.logger.info(f"定时分析开始: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        self.logger.info("=" * 60)

        try:
            # 1. 执行批量分析
            self.logger.info("[1/4] 执行持仓批量分析...")
            analysis_results = portfolio_manager.batch_analyze_portfolio(
                mode=self.analysis_mode, max_workers=self.max_workers, selected_agents=self.selected_agents
            )

            if not analysis_results.get("success"):
                error_msg = analysis_results.get("error", "未知错误")
                self.logger.error(f"批量分析失败: {error_msg}")
            ...
            # 2. 保存分析结果
            self.logger.info("[2/4] 保存分析结果...")
            saved_ids = portfolio_manager.save_analysis_results(analysis_results)
            self.logger.info(f"保存 {len(saved_ids)} 条分析记录")

            # 3. 自动监测同步
            sync_result = None
            if self.auto_monitor_sync:
                self.logger.info("[3/4] 自动同步到监测列表...")
                sync_result = self._sync_to_monitor(analysis_results)
            else:
                self.logger.info("[3/4] 跳过监测同步（已禁用）")

            # 4. 发送通知
            if self.notification_enabled:
                self.logger.info("[4/4] 发送通知...")
                self._send_notification(analysis_results, sync_result)
            else:
                self.logger.info("[4/4] 跳过通知发送（已禁用）")

            # 更新运行时间
            self.last_run_time = datetime.now()

            self.logger.info("=" * 60)
            self.logger.info(f"定时分析完成: {self.last_run_time.strftime('%Y-%m-%d %H:%M:%S')}")
            self.logger.info("=" * 60)

        except Exception as e:
            self.logger.error(f"定时任务执行异常: {str(e)}", exc_info=True)
        ...
        try:
            ...
            for item in analysis_results.get("results", []):
                ...
                if not all([entry_min, entry_max, take_profit, stop_loss]):
                    self.logger.warning(f"{code} 参数不完整，跳过同步")
                    failed_count += 1
                    continue
            ...
            if monitors_data:
                ...
                self.logger.warning("没有需要同步的监测数据")
                return {"added": 0, "updated": 0, "failed": 0, "total": 0}

        except Exception as e:
            self.logger.error(f"监测同步异常: {str(e)}", exc_info=True)
            return {"added": 0, "updated": 0, "failed": 0, "total": 0}
    ...
    def _send_notification(self, analysis_results: dict, sync_result: Optional[dict]):
        ...
        try:
            ...
            # 使用新的专用通知方法
            success = notification_service.send_portfolio_analysis_notification(analysis_results, sync_result)

            if success:
                self.logger.info("持仓分析通知发送成功")
            else:
                self.logger.warning("持仓分析通知发送失败（可能未配置通知服务）")
...
"""
    ...
    def _generate_notification_content(self, analysis_results: dict, sync_result: Optional[dict]) -> str:
        """
        生成通知内容
...
"""
    ...
    def _reschedule(self):
        ...
        jobs_to_remove = [
            job for job in schedule.jobs if not any(tag in ["sector_strategy", "monitor"] for tag in job.tags)
        ]
        ...
        self.logger.info(f"重新调度任务: 每天 {', '.join(self.schedule_times)}")
    ...
    def start(self) -> bool:
        ...
        if stock_count == 0:
            self.logger.error("没有持仓股票，无法启动定时任务")
            return False

        # 检查时间配置
        if not self.schedule_times:
            self.logger.error("没有配置定时时间")
            return False
        ...
        for job in jobs_to_remove:
            schedule.cancel_job(job)
        self.logger.info(f"清除了 {len(jobs_to_remove)} 个旧的持仓任务")

        for time_str in self.schedule_times:
            job = schedule.every().day.at(time_str).do(self._scheduled_job)
            job.tag("portfolio_analysis")
            self.logger.info(f"添加调度任务: 每天 {time_str}")
        ...
        self.logger.info(f"    调度时间: {', '.join(self.schedule_times)}")
        self.logger.info(f"    分析模式: {self.analysis_mode}")
        self.logger.info(f"    持仓数量: {stock_count}只")
        if self.next_run_time:
            self.logger.info(f"    下次运行: {self.next_run_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            for job in jobs_to_remove:
                schedule.cancel_job(job)
            self.logger.info(f"清除了 {len(jobs_to_remove)} 个持仓任务")
        except Exception as e:
            self.logger.warning(f"清除任务时出错: {e}")
    ...
    def run_once(self) -> bool:
        ...
        stock_count = portfolio_manager.get_stock_count()
        if stock_count == 0:
            self.logger.error("没有持仓股票")
            return False

        self.logger.info("立即执行持仓分析...")
        self._scheduled_job()
        return True
    ...
    def update_config(
        ...
        if max_workers is not None:
            ...
            self.logger.info(f"设置并行线程数: {max_workers}")
