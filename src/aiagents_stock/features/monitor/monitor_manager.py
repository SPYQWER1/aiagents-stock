...
def display_monitored_stocks():
    ...
    if search_term:
        filtered_stocks = [
            s for s in stocks if search_term.lower() in s["symbol"].lower() or search_term.lower() in s["name"].lower()
        ]
...
def display_edit_dialog(stock_id: int):
    ...
    with st.form(key=f"edit_form_{stock_id}"):
        ...
        with col1:
            st.subheader("🎯 关键位置")
            entry_range = stock.get("entry_range", {})
            entry_min = st.number_input(
                "进场区间最低价", value=float(entry_range.get("min", 0)), step=0.01, format="%.2f"
            )
            entry_max = st.number_input(
                "进场区间最高价", value=float(entry_range.get("max", 0)), step=0.01, format="%.2f"
            )
            take_profit = st.number_input(
                "止盈价位", value=float(stock["take_profit"]) if stock["take_profit"] else 0.0, step=0.01, format="%.2f"
            )
            stop_loss = st.number_input(
                "止损价位", value=float(stock["stop_loss"]) if stock["stop_loss"] else 0.0, step=0.01, format="%.2f"
            )
        ...
        with col2:
            ...
            rating = st.selectbox(
                ...
                ["买入", "持有", "卖出"],
                index=(
                    ["买入", "持有", "卖出"].index(stock["rating"])
                    if stock["rating"] in ["买入", "持有", "卖出"]
                    else 0
                ),
            )
            ...
            if quant_enabled:
                ...
                max_position_pct = st.slider("最大仓位比例", 0.05, 0.5, quant_config.get("max_position_pct", 0.2), 0.05)
    ...
    with col2:
        ...
        if all_notifications:
            ...
            for notification in all_notifications:
                ...
                color_map = {"entry": "🟢", "take_profit": "🟡", "stop_loss": "🔴", "quant_trade": "🤖"}
                ...
                # 显示通知信息
                st.info(
                    f"{icon} **{notification['symbol']}** - {notification['message']}\n\n_{notification['triggered_at']}_ | {sent_status}"
                )
...
def display_scheduler_section():
    ...
    with st.expander("⚙️ 定时调度设置", expanded=False):
        ...
        with col1:
            ...
            market_names = {"CN": "中国A股", "US": "美股", "HK": "港股"}
        ...
        with col2:
            ...
            enabled = st.checkbox(
                "启用定时调度", value=scheduler.config.get("enabled", False), help="启用后将在交易时间自动启动监测服务"
            )

            auto_stop = st.checkbox(
                "收盘后自动停止", value=scheduler.config.get("auto_stop", True), help="在交易时间结束后自动停止监测服务"
            )
