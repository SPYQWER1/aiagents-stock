...
from aiagents_stock.features.profit_growth.profit_growth_monitor import profit_growth_monitor
from aiagents_stock.features.profit_growth.profit_growth_selector import profit_growth_selector
...
def add_stock_to_monitor_button(stock_code: str, stock_name: str, price: float = None):
    ...
    if st.button("➕ 加入策略监控", key=button_key, width="stretch"):
        ...
        success, message = profit_growth_monitor.add_stock(
            stock_code=stock_code, stock_name=stock_name, buy_price=price
        )
...
def display_history():
    ...
    with sub_tab1:
        ...
        if alerts:
            ...
            for alert in alerts:
                st.markdown(
                    f"- **{alert['alert_time']}** | {alert['stock_code']} {alert['stock_name']} | {alert['alert_type']}"
                )
        else:
            st.info("📭 暂无提醒历史")
    ...
    with sub_tab2:
        ...
        if removed:
            ...
            for stock in removed:
                st.markdown(
                    f"- **{stock['remove_time']}** | {stock['stock_code']} {stock['stock_name']} | {stock['remove_reason']}"
                )
        else:
            st.info("📭 暂无移除历史")
