...
class LowPriceBullService:
    ...
    def _get_stock_data(self, stock_code: str) -> tuple:
        ...
        try:
            ...
            params = {"code": api_code, "type": "day"}  # 日K线
        ...
        try:
            ...
            for alert in alerts:
                try:
                    ...
                    # 自动移除股票
                    low_price_bull_monitor.remove_stock(alert["stock_code"], reason=alert["alert_reason"])
    ...
    def _send_alert_notification(self, alert: dict):
        ...
        try:
            ...
            if notification_service.config["webhook_enabled"]:
                ...
                data = {"msgtype": "markdown", "markdown": {"title": f"{keyword} - 卖出提醒", "text": message_text}}
