...
class LowPriceBullSelector:
    ...
    def get_low_price_stocks(self, top_n: int = 5) -> Tuple[bool, Optional[pd.DataFrame], str]:
        ...
        try:
            ...
            self.logger.info(f"✅ 成功获取 {len(df_result)} 只股票")

            # 显示获取到的列名
            self.logger.info("获取到的数据字段:")
            ...
            if len(df_result) > top_n:
                ...
                self.logger.info(f"从 {len(df_result)} 只股票中选出前 {top_n} 只")
            else:
                selected = df_result
                self.logger.info(f"共 {len(df_result)} 只符合条件的股票")

            self.selected_stocks = selected

            # 显示选中的股票
            self.logger.info("✅ 选中的股票:")
            for idx, row in selected.iterrows():
                ...
                self.logger.info(f"  {idx+1}. {code} {name} - 股价:{price} 净利增长:{growth}% 成交额:{turnover}")

            self.logger.info("选股结束")
    ...
    def _convert_to_dataframe(self, result) -> Optional[pd.DataFrame]:
        ...
        try:
            if isinstance(result, pd.DataFrame):
                ...
                self.logger.warning(f"⚠️ 未知的数据格式: {type(result)}")
                return None
        except Exception as e:
            self.logger.error(f"转换DataFrame失败: {e}", exc_info=True)
            return None
