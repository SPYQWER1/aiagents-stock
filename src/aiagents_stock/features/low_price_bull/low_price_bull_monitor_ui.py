...
from aiagents_stock.features.low_price_bull.low_price_bull_monitor import low_price_bull_monitor
from aiagents_stock.features.low_price_bull.low_price_bull_service import low_price_bull_service
...
def display_monitored_stocks():
    ...
    selected_codes = st.multiselect(
        "选择要移除的股票", options=[f"{s['stock_code']} {s['stock_name']}" for s in stocks], format_func=lambda x: x
    )
...
def display_sell_alerts():
    ...
    for alert in alerts:
        with st.expander(f"🔔 {alert['stock_code']} {alert['stock_name']} - {alert['alert_reason']}", expanded=True):
...
def _get_alert_type_name(alert_type: str) -> str:
    """获取提醒类型名称"""
    type_map = {"holding_days": "持股到期", "ma_cross": "MA均线死叉"}
