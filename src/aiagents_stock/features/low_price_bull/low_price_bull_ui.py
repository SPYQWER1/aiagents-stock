...
from aiagents_stock.features.low_price_bull.low_price_bull_selector import LowPriceBullSelector
from aiagents_stock.features.low_price_bull.low_price_bull_strategy import LowPriceBullStrategy
...
def display_low_price_bull():
    ...
    if st.session_state.get("show_low_price_monitor"):
        from aiagents_stock.features.low_price_bull.low_price_bull_monitor_ui import display_monitor_panel
    ...
    **量化交易策略**：
    ...
    with col1:
        top_n = st.slider("筛选数量", min_value=3, max_value=10, value=5, step=1, help="选择展示的股票数量")
    ...
    if "low_price_bull_stocks" in st.session_state:
        display_stock_results(st.session_state.low_price_bull_stocks, st.session_state.get("low_price_bull_selector"))
...
def display_stock_results(stocks_df: pd.DataFrame, selector):
    ...
    for idx, row in stocks_df.iterrows():
        ...
        with st.expander(f"【第{idx+1}名】{code} - {name}{price_str}", expanded=(idx < 3)):
...
def display_stock_detail(row: pd.Series):
    ...
    from aiagents_stock.features.low_price_bull.low_price_bull_monitor_ui import add_stock_to_monitor_button
...
def run_strategy_simulation(stocks_df: pd.DataFrame):
    ...
    for idx, row in stocks_df.head(strategy.max_daily_buy).iterrows():
        ...
        if price > 0:
            ...
            buy_results.append({"success": success, "message": message, "trade": trade})
...
def send_dingtalk_notification(stocks_df: pd.DataFrame, top_n: int):
    ...
    try:
        ...
        message_text += "**筛选策略**: 股价<10元 + 净利润增长率≥100% + 沪深A股\n\n"
        ...
        if notification_service.config["webhook_type"] == "dingtalk":
            ...
            data = {"msgtype": "markdown", "markdown": {"title": f"{keyword}", "text": message_text}}
