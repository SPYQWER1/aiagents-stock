...
class LowPriceBullMonitor:
    ...
    def add_stock(self, stock_code: str, stock_name: str, buy_price: float, buy_date: str = None) -> Tuple[bool, str]:
        """
        添加股票到监控列表
        ...
        try:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
            """,
                (stock_code, stock_name, buy_price, buy_date, datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
            )
