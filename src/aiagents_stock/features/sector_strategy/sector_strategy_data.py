...
class SectorStrategyDataFetcher:
    ...
    def __init__(self):
        ...
        self.logger.info("[智策] 板块数据获取器初始化...")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    self.logger.error(f"请求失败，已达最大重试次数: {e}", exc_info=True)
                    raise e
    ...
    def get_all_sector_data(self):
        ...
        try:
            ...
            self.logger.error(f"[智策] ✗ 数据获取出错: {e}", exc_info=True)
            data["error"] = str(e)
    ...
    def _get_sector_performance(self):
        ...
        try:
            ...
            self.logger.error(f"    获取行业板块数据失败: {e}", exc_info=True)
            return {}
    ...
    def _get_concept_performance(self):
        ...
        try:
            ...
            self.logger.error(f"    获取概念板块数据失败: {e}", exc_info=True)
            return {}
    ...
    def _get_sector_fund_flow(self):
        ...
        try:
            ...
            # 转换为字典格式
            fund_flow = {"today": [], "update_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    ...
    def _get_north_money_flow(self):
        ...
        try:
            ...
            if not hasattr(self, "_tushare_api"):
                ...
                if TUSHARE_TOKEN:
                    try:
                        ...
                        self.logger.error(f"    [Tushare] 初始化失败: {e}", exc_info=True)
                        self._tushare_api = None
                else:
                    self.logger.warning("    [Tushare] 未配置Token")
                    self._tushare_api = None
            ...
            if hasattr(self, "_tushare_api") and self._tushare_api:
                ...
                if df is not None and not df.empty:
                    self.logger.info("    [Tushare] ✅ 成功获取数据")
                    ...
                    north_flow = {
                        ...
                        "north_total_amount": float(
                            latest["north_money"]
                        ),  # Tushare没有总成交金额，使用净流入作为近似值
                    }

                    # 获取历史趋势（最近20天）
                    history = []
                    for idx, row in df.head(20).iterrows():
                        history.append({"date": str(row["trade_date"]), "net_inflow": float(row["north_money"])})
                    north_flow["history"] = history

                    return north_flow
                else:
                    self.logger.warning("    [Tushare] ❌ 未获取到数据")
            else:
                self.logger.warning("    [Tushare] 不可用")
        except Exception as e:
            self.logger.error(f"    [Tushare] 获取北向资金失败: {e}", exc_info=True)
        ...
        try:
            ...
            if df is not None and not df.empty:
                self.logger.info("    [Akshare] ✅ 成功获取数据")
                ...
                history = []
                for idx, row in df.head(20).iterrows():
                    history.append({"date": str(row.get("日期", "")), "net_inflow": row.get("北向资金-成交净买额", 0)})
                north_flow["history"] = history

                return north_flow
            else:
                self.logger.warning("    [Akshare] ❌ 未获取到数据")
        except Exception as e:
            self.logger.error(f"    [Akshare] 获取北向资金失败: {e}", exc_info=True)
    ...
    def _get_financial_news(self):
        ...
        try:
            ...
            self.logger.error(f"    获取财经新闻失败: {e}", exc_info=True)
            return []
...
""")
            for name, info in sorted_sectors[:10]:
                text_parts.append(
                    f"  {name}: {info['change_pct']:+.2f}% | 领涨: {info['top_stock']} ({info['top_stock_change']:+.2f}%)"
                )
...
""")
            for name, info in sorted_sectors[-10:]:
                text_parts.append(
                    f"  {name}: {info['change_pct']:+.2f}% | 领跌: {info['top_stock']} ({info['top_stock_change']:+.2f}%)"
                )
...
""")
            ...
            for item in sorted_flow[:15]:
                text_parts.append(
                    f"  {item['sector']}: {item['main_net_inflow']:.2f}万 ({item['main_net_inflow_pct']:+.2f}%) | 涨跌: {item['change_pct']:+.2f}%"
                )
...
""")
        ...
        try:
            ...
            if data.get("sectors"):
                ...
                self.database.save_sector_raw_data(
                    data_date=datetime.now().strftime("%Y-%m-%d"), data_type="industry", data_df=sectors_df
                )
            ...
            if data.get("concepts"):
                ...
                self.database.save_sector_raw_data(
                    data_date=datetime.now().strftime("%Y-%m-%d"), data_type="concept", data_df=concepts_df
                )
            ...
            if data.get("sector_fund_flow"):
                ...
                if not fund_df.empty:
                    self.database.save_sector_raw_data(
                        data_date=datetime.now().strftime("%Y-%m-%d"), data_type="fund_flow", data_df=fund_df
                    )
            ...
            if data.get("market_overview"):
                ...
                mo_df = pd.DataFrame(
                    [
                        {
                            "名称": "上证指数",
                            "最新价": market.get("sh_index", {}).get("close", 0),
                            "涨跌幅": market.get("sh_index", {}).get("change_pct", 0),
                            "成交量": market.get("sh_index", {}).get("volume", 0),
                            "成交额": market.get("sh_index", {}).get("turnover", 0),
                        },
                        {
                            "名称": "深证成指",
                            "最新价": market.get("sz_index", {}).get("close", 0),
                            "涨跌幅": market.get("sz_index", {}).get("change_pct", 0),
                            "成交量": market.get("sz_index", {}).get("volume", 0),
                            "成交额": market.get("sz_index", {}).get("turnover", 0),
                        },
                        {
                            "名称": "创业板指",
                            "最新价": market.get("cyb_index", {}).get("close", 0),
                            "涨跌幅": market.get("cyb_index", {}).get("change_pct", 0),
                            "成交量": market.get("cyb_index", {}).get("volume", 0),
                            "成交额": market.get("cyb_index", {}).get("turnover", 0),
                        },
                    ]
                )
                self.database.save_sector_raw_data(
                    data_date=datetime.now().strftime("%Y-%m-%d"), data_type="market_overview", data_df=mo_df
                )
            ...
            if data.get("news"):
                self.database.save_news_data(
                    news_list=data["news"], news_date=datetime.now().strftime("%Y-%m-%d"), source="akshare"
                )
    ...
    def get_cached_data_with_fallback(self):
        ...
        try:
            ...
            # 如果获取失败，回退到缓存数据
            self.logger.warning("[智策] 获取最新数据失败，尝试加载缓存数据...")
            ...
            if cached_data:
                ...
                self.logger.warning("[智策] ✗ 无可用缓存数据")
                return {
                    "success": False,
                    "error": "无法获取数据且无可用缓存",
                    "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ...
            self.logger.error(f"[智策数据] 获取数据失败: {e}", exc_info=True)
