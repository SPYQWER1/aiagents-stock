...
class SectorStrategyDatabase:
    ...
    def save_raw_data(self, data_date, data_type, data_df, version=None):
        ...
        try:
            ...
            conn.commit()
            self.logger.info(
                f"[智策板块] 保存{data_type}数据成功 (日期: {data_date}, 版本: {version}, 记录数: {len(data_df)})"
            )
            return version
    ...
    def get_latest_data(self, data_type, data_date=None):
        ...
        try:
            ...
            self.logger.info(
                f"[智策板块] 获取{data_type}数据成功 (日期: {data_date}, 版本: {version}, 记录数: {len(data_df)})"
            )
            return data_df
    ...
    def save_sector_raw_data(self, data_date, data_type, data_df):
        ...
        try:
            ...
            conn.commit()
            self.logger.info(
                f"[智策板块] {data_type}数据保存成功 (日期: {data_date}, 版本: {version}, 记录数: {len(data_df)})"
            )
            ...
            """,
                (
                    ...
                    (
                        ...
                        if pd.notna(row.get("最新价", row.get("price", 0)))
                        else 0
                    ),
                    (
                        float(row.get("涨跌幅", row.get("change_pct", 0)))
                        if pd.notna(row.get("涨跌幅", row.get("change_pct", 0)))
                        else 0
                    ),
                    (
                        float(row.get("成交量", row.get("volume", 0)))
                        if pd.notna(row.get("成交量", row.get("volume", 0)))
                        else 0
                    ),
                    (
                        float(row.get("成交额", row.get("turnover", 0)))
                        if pd.notna(row.get("成交额", row.get("turnover", 0)))
                        else 0
                    ),
                    (
                        float(row.get("总市值", row.get("market_cap", 0)))
                        if pd.notna(row.get("总市值", row.get("market_cap", 0)))
                        else 0
                    ),
                    (
                        float(row.get("市盈率", row.get("pe_ratio", 0)))
                        if pd.notna(row.get("市盈率", row.get("pe_ratio", 0)))
                        else 0
                    ),
                    (
                        float(row.get("市净率", row.get("pb_ratio", 0)))
                        if pd.notna(row.get("市净率", row.get("pb_ratio", 0)))
                        else 0
                    ),
                    data_type,
                    version,
    ...
    def get_latest_raw_data(self, key: str, within_hours: int = 24):
        ...
        try:
            ...
            """,
                conn,
                params=[data_type, data_date, version],
            )
    ...
    def get_latest_news_data(self, within_hours: int = 24):
        ...
        try:
            ...
            return {"data_date": df.iloc[0]["news_date"] if not df.empty else None, "data_content": news}
        except Exception as e:
