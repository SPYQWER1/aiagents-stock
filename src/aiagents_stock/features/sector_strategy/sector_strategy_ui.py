...
from aiagents_stock.features.sector_strategy.sector_strategy_pdf import SectorStrategyPDFGenerator
from aiagents_stock.features.sector_strategy.sector_strategy_scheduler import sector_strategy_scheduler
...
def display_data_summary(data):
    ...
    with col1:
        if market.get("sh_index"):
            sh = market["sh_index"]
            st.metric("上证指数", f"{sh['close']:.2f}", f"{sh['change_pct']:+.2f}%")

    with col2:
        if market.get("up_count"):
            st.metric("上涨股票", market["up_count"], f"{market['up_ratio']:.1f}%")
...
def display_analysis_results(result):
    ...
    # 创建标签页
    tab1, tab2, tab3, tab4 = st.tabs(["📋 核心预测", "🤖 智能体分析", "📊 综合研判", "📈 数据可视化"])
...
def display_predictions(predictions):
    ...
    with col1:
        ...
        for idx, item in enumerate(hottest, 1):
            st.metric(f"{idx}. {item.get('sector', 'N/A')}", f"{item.get('score', 0)}分", f"{item.get('trend', 'N/A')}")

    with col2:
        st.markdown("#### 📈 升温板块")
        heating = heat.get("heating", [])
        for idx, item in enumerate(heating, 1):
            st.metric(f"{idx}. {item.get('sector', 'N/A')}", f"{item.get('score', 0)}分", "↗️ 升温")

    with col3:
        st.markdown("#### 📉 降温板块")
        cooling = heat.get("cooling", [])
        for idx, item in enumerate(cooling, 1):
            st.metric(f"{idx}. {item.get('sector', 'N/A')}", f"{item.get('score', 0)}分", "↘️ 降温")
...
def display_visualizations(predictions):
    ...
    if bullish or bearish:
        ...
        # 创建条形图
        df = pd.DataFrame({"板块": sectors, "信心度": confidence, "类型": types})
    ...
    if hottest or heating:
        ...
        df = pd.DataFrame({"板块": sectors, "热度": scores, "趋势": trends})
        ...
        **前提条件：**
        ...
        with col2:
            ...
            schedule_time = st.time_input(
                "设置定时时间", value=dt_time(9, 0), help="系统将在每天此时间自动运行分析"  # 默认9:00
            )
