...
from aiagents_stock.features.sector_strategy.sector_strategy_data import SectorStrategyDataFetcher
from aiagents_stock.features.sector_strategy.sector_strategy_engine import SectorStrategyEngine
...
class SectorStrategyScheduler:
    ...
    def start(self, schedule_time="09:00"):
        ...
        try:
            ...
            self.logger.error(f"[智策定时] 清除旧任务时出错: {e}", exc_info=True)
    ...
    def _schedule_loop(self):
        ...
        while self.running:
            try:
                ...
                self.logger.error(f"[智策定时] ✗ 调度循环出错: {e}", exc_info=True)
                time.sleep(60)
    ...
    def _run_analysis(self):
        ...
        try:
            ...
            self.logger.error(f"[智策定时] ✗ 分析过程出错: {e}", exc_info=True)
    ...
    def _send_analysis_notification(self, result):
        ...
        try:
            ...
            self.logger.error(f"[智策定时] ✗ 通知发送异常: {e}", exc_info=True)
...
"""
    ...
    def _send_webhook_direct(self, predictions, timestamp):
        ...
        try:
            ...
            self.logger.error(f"[智策定时] ✗ Webhook发送失败: {e}", exc_info=True)
            return False
    ...
    def _send_dingtalk(self, url, summary, timestamp):
        ...
        try:
            ...
            data = {"msgtype": "markdown", "markdown": {"title": f"{title_prefix}智策板块分析报告", "text": summary}}
    ...
    def _send_feishu(self, url, summary, timestamp):
        ...
        try:
            ...
            data = {
                "msg_type": "text",
                "content": {"text": f"{title_prefix}智策板块分析报告】\n分析时间: {timestamp}\n\n{summary}"},
            }
    ...
    def _format_webhook_summary(self, predictions, timestamp):
        ...
        if long_short:
            ...
            if bullish or bearish:
                ...
                if bullish:
                    lines.append(
                        "**看多**: "
                        + "、".join([f"{item.get('sector')}({item.get('confidence')}分)" for item in bullish[:3]])
                    )
                if bearish:
                    lines.append(
                        "**看空**: "
                        + "、".join([f"{item.get('sector')}({item.get('confidence')}分)" for item in bearish[:3]])
                    )
                lines.append("")
        ...
        if summary and summary.get("key_opportunity"):
            lines.append("#### 💡 核心机会")
            lines.append(
                summary["key_opportunity"][:150] + "..."
                if len(summary.get("key_opportunity", "")) > 150
                else summary.get("key_opportunity", "")
            )
            lines.append("")
    ...
    def _send_email_direct(self, subject, body):
        ...
        try:
            ...
            self.logger.info("[智策定时]   - 正在登录...")
            server.login(config["email_from"], config["email_password"])
            self.logger.info("[智策定时]   - 正在发送...")
            ...
            self.logger.error(f"[智策定时] ✗ 邮件发送失败: {e}", exc_info=True)
            return False
...
"""
        ...
        body_parts.append("AI模型: DeepSeek Multi-Agent System")
        body_parts.append("")
        ...
        if heat:
            ...
            if hottest:
                ...
                for idx, item in enumerate(hottest, 1):
                    body_parts.append(
                        f"{idx}. {item.get('sector', 'N/A')} - 热度: {item.get('score', 0)}分 ({item.get('trend', 'N/A')})"
                    )
                body_parts.append("")
