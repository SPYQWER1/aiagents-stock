...
# 页面配置
st.set_page_config(
    page_title="复合多AI智能体股票团队分析系统", page_icon="📈", layout="wide", initial_sidebar_state="expanded"
)
...
def main():
    ...
    with st.sidebar:
        ...
        # 分析参数设置
        st.markdown("### 📊 分析参数")
        period = st.selectbox("数据周期", ["1y", "6mo", "3mo", "1mo"], index=0, help="选择历史数据的时间范围")
...
def run_batch_analysis(stock_list, period, batch_mode="顺序分析"):
            ...
            for future in concurrent.futures.as_completed(future_to_symbol):
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        status_text.text(
                            f"❌ [{len(results)}/{total}] {symbol} 分析失败: {result.get('error', '未知错误')}"
                        )
    ...
    if success_count > 0:
        status_text.success(
            f"✅ 批量分析完成！成功 {success_count} 只，失败 {failed_count} 只，已保存 {saved_count} 只到历史记录"
        )
        ...
        if enable_fundamental and fetcher._is_chinese_stock(symbol):
            ...
            try:
                ...
                if quarterly_data and quarterly_data.get("data_success"):
                    income_count = (
                        quarterly_data.get("income_statement", {}).get("periods", 0)
                        if quarterly_data.get("income_statement")
                        else 0
                    )
                    balance_count = (
                        quarterly_data.get("balance_sheet", {}).get("periods", 0)
                        if quarterly_data.get("balance_sheet")
                        else 0
                    )
                    cash_flow_count = (
                        quarterly_data.get("cash_flow", {}).get("periods", 0) if quarterly_data.get("cash_flow") else 0
                    )
                    st.info(
                        f"✅ 成功获取季报数据：利润表{income_count}期，资产负债表{balance_count}期，现金流量表{cash_flow_count}期"
                    )
                else:
        ...
        if enable_fund_flow and fetcher._is_chinese_stock(symbol):
            ...
            try:
                ...
                if fund_flow_data and fund_flow_data.get("data_success"):
                    days = (
                        fund_flow_data.get("fund_flow_data", {}).get("days", 0)
                        if fund_flow_data.get("fund_flow_data")
                        else 0
                    )
...
def display_stock_chart(stock_data, stock_info):
    ...
    if "MA5" in stock_data.columns:
        fig.add_trace(
            go.Scatter(x=stock_data.index, y=stock_data["MA5"], name="MA5", line=dict(color="orange", width=1))
        )

    if "MA20" in stock_data.columns:
        fig.add_trace(
            go.Scatter(x=stock_data.index, y=stock_data["MA20"], name="MA20", line=dict(color="blue", width=1))
        )

    if "MA60" in stock_data.columns:
        fig.add_trace(
            go.Scatter(x=stock_data.index, y=stock_data["MA60"], name="MA60", line=dict(color="purple", width=1))
        )
    ...
    if "Volume" in stock_data.columns:
        ...
        fig_volume.add_trace(
            go.Bar(x=stock_data.index, y=stock_data["Volume"], name="成交量", marker_color="lightblue")
        )
...
def display_history_records():
    ...
    for record in filtered_records:
        ...
        rating_color = {"买入": "🟢", "持有": "🟡", "卖出": "🔴", "强烈买入": "🟢", "强烈卖出": "🔴"}.get(rating, "⚪")
...
def display_config_manager():
    ...
    with tab1:
        ...
        if new_api_key:
            masked_key = (
                new_api_key[:8] + "*" * (len(new_api_key) - 12) + new_api_key[-4:] if len(new_api_key) > 12 else "***"
            )
    ...
    with tab3:
        ...
        new_enabled = st.checkbox(
            "启用MiniQMT量化交易", value=current_enabled, help="开启后可以使用量化交易功能", key="input_miniqmt_enabled"
        )
        ...
        with col1:
            ...
            new_host = st.text_input(
                f"🖥️ {host_info['description']}", value=current_host, disabled=not new_enabled, key="input_miniqmt_host"
            )
        ...
        with col2:
            ...
            new_port = st.text_input(
                f"🔌 {port_info['description']}", value=current_port, disabled=not new_enabled, key="input_miniqmt_port"
            )
    ...
    with tab4:
        ...
        with col_email:
            ...
            new_email_enabled = st.checkbox(
                "启用邮件通知", value=current_email_enabled, help="开启后可以接收邮件提醒", key="input_email_enabled"
            )
        ...
        with col_webhook:
            ...
            # Webhook类型选择
            webhook_type_info = config_info.get(
                "WEBHOOK_TYPE", {"description": "Webhook类型", "value": "dingtalk", "options": ["dingtalk", "feishu"]}
            )
            ...
            webhook_keyword_info = config_info.get(
                "WEBHOOK_KEYWORD", {"description": "自定义关键词（钉钉安全验证）", "value": "aiagents通知"}
            )
