...
class MiniQMTInterface:
    ...
    def get_account_info(self) -> Dict:
        ...
        if not self.is_connected():
            return {"error": "未连接到MiniQMT", "connected": False}
    ...
    def execute_strategy_signal(
        self, stock_id: int, symbol: str, signal: Dict, position_size: float = 0.2
    ) -> Tuple[bool, str]:
        """
        执行策略信号
        根据监测触发的信号自动执行交易
        ...
        try:
            ...
            if signal_type == "entry":
                ...
                if position and position.get("quantity", 0) > 0:
                    ...
                    # 止损使用市价单，快速成交
                    success, msg, order_id = self.place_order(
                        symbol=symbol, action=TradeAction.SELL, quantity=quantity, order_type=OrderType.MARKET
                    )
    ...
    def calculate_position_size(
        self, symbol: str, price: float, max_position_pct: float = 0.2, max_risk_pct: float = 0.02
    ) -> int:
        """
        计算建议仓位大小
    ...
    def get_risk_metrics(self, symbol: str) -> Dict:
        ...
        position = self.get_position(symbol)
        if not position:
            return {"has_position": False, "profit_loss": 0, "profit_loss_pct": 0, "risk_exposure": 0}
    ...
    def validate_trade(self, symbol: str, action: TradeAction, quantity: int, price: float = None) -> Tuple[bool, str]:
        """
        验证交易是否可行
...
def init_miniqmt(config: Dict = None) -> Tuple[bool, str]:
    ...
    try:
        ...
        if config is None:
            try:
                from aiagents_stock.core.config import MINIQMT_CONFIG

                config = MINIQMT_CONFIG
            except ImportError:
                config = {"enabled": False, "account_id": None}
