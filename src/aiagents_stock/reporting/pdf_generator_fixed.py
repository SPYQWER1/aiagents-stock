...
def generate_html_content(markdown_content: str) -> str:
    safe_content = html.escape(markdown_content)
...
<head>
  ...
  <style>
    body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, 'Noto Sans', 'PingFang SC', 'Hiragino Sans GB', 'Microsoft YaHei', sans-serif; margin: 24px; line-height: 1.6; }}
        ...
        if st.button("📊 生成并下载报告(PDF/HTML)", type="primary", width="content", key=pdf_button_key):
            with st.spinner("正在生成报告..."):
                try:
                    ...
                    # 创建下载链接
                    md_link = create_download_link(markdown_content, f"{filename}.md", "📝 下载Markdown报告")

                    html_link = create_html_download_link(html_content, f"{filename}.html", "🌐 下载HTML报告")
        ...
        if st.button("📝 生成并下载Markdown报告", type="secondary", width="content", key=markdown_button_key):
            with st.spinner("正在生成Markdown报告..."):
                try:
                    ...
                    # 创建下载链接
                    md_link = create_download_link(markdown_content, filename, "📝 下载Markdown报告")
