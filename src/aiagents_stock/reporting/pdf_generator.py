        ...
        for font_path in all_font_paths:
            if os.path.exists(font_path):
                try:
                    ...
                    logger.warning(f"⚠️ 尝试注册字体 {font_path} 失败: {e}")
                    continue

        # 如果没有找到中文字体，打印警告并使用默认字体
        logger.warning("⚠️ 警告：未找到中文字体，PDF中文可能显示为方框")
        logger.warning("建议：在Docker中安装中文字体包")
        return "Helvetica"
    except Exception as e:
        logger.error(f"❌ 注册中文字体时出错: {e}", exc_info=True)
        return "Helvetica"
...
def create_pdf_report(stock_info, agents_results, discussion_result, final_decision):
    ...
    normal_style = ParagraphStyle(
        "CustomNormal", parent=styles["Normal"], fontName=chinese_font, fontSize=11, spaceAfter=6, alignment=TA_JUSTIFY
    )
